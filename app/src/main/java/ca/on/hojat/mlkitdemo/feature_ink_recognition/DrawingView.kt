...
import ca.on.hojat.mlkitdemo.feature_ink_recognition.StrokeManager.ContentChangedListener
...
class DrawingView @JvmOverloads constructor(
    ...
    companion object {
        ...
        private fun computeBoundingBox(ink: Ink): Rect {
            ...
            for (s in ink.strokes) {
                for (p in s.points) {
                    top = top.coerceAtMost(p.y)
                    left = left.coerceAtMost(p.x)
                    bottom = bottom.coerceAtLeast(p.y)
                    right = right.coerceAtLeast(p.x)
                }
            }
