...
class ObjectGraphic constructor(
    overlay: GraphicOverlay, private val detectedObject: DetectedObject
    ...
    override fun draw(canvas: Canvas) {
        ...
        for (label in detectedObject.labels) {
            ...
            textWidth = max(
                textWidth, textPaints[colorID].measureText(
                    String.format(
        ...
        for (label in detectedObject.labels) {
            ...
            canvas.drawText(
                String.format(
                    Locale.US, LABEL_FORMAT, label.confidence * 100, label.index
                ), rect.left, rect.top + yLabelOffset, textPaints[colorID]
            )
            yLabelOffset += lineHeight
        }
    }
